...
"""
@file check_create_internal_subnet.py
"""
...
class CreateNetWork(unittest.TestCase):
    ...
    @ddt.unpack
    def test_create_internal_network(self, net_name, external_router, id):
        net_ret = ControllerTest.create_network(net_name, external_router, id)
        sub_net_name = "sub-" + net_name
        if re.findall('ACTIVE', net_ret):
            sub_net_ret = ControllerTest.create_internal_subnet(sub_net_name,
                                                                net_name)
