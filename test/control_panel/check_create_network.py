...
class CreateNetWork(unittest.TestCase):
    ...
    @class CreateNetWork
    '''
    @ddt.data(["autotest-internal", "False", ""],
              ["autotest-external", "True", "374"],
              )
    @ddt.unpack
    def test_create_external_network(self, net_name, external_router, id):
        output = ControllerTest.create_network(net_name, external_router, id)
        if re.findall('ACTIVE', output):
            pass
            ControllerTest.delete_network(net_name)
        else:
            assert False, "create %s fail, please check if there is too much \
networks or other conflict" % net_name
